...
public class FriendService {
	...
	public ResponseResult delFriendRelation(String user_mobile, Long friend_id) {
		...
		UserFriendRelation userFriendRelation = userFriendRelationRepository.findByUser_idAndFriend_id(user.getId(),
				friend_id);
		UserFriendRelation userFriendRelationReverse = userFriendRelationRepository.findByUser_idAndFriend_id(friend_id,
				user.getId());
		if (userFriendRelation == null) {
			logger.error("[delFriendRelation] userFriendRelation: [" + user.getId() + ", " + friend_id
					+ "] doesn't exist; ");
	...
	public ResponseResult acceptUserFriendRequest(String user_mobile, Long friend_id) {
		...
		try {
			UserRequestFriend userRequestFriend = userRequestFriendRepository.findByUser_idAndFriend_id(friend_id,
					user.getId());
		...
		PushMessage<User> pushMessage = new PushMessage<User>(101, user);
		return new ResponseResult(true, JsonConverterUtil.convertObjToString(pushMessage));
	}
	...
	public ResponseResult addTaggedFriend(String user_mobile, Long tag_id, Long friend_id) {
		...
		if (userFriendRelation == null) {
			logger.error(
					"[addTaggedFriend] userFriendRelation: [" + user.getId() + ", " + friend_id + "] doesn't exist; ");
			throw new FriendRelationNotFoundException(user.getId(), friend_id);
		}
		try {
	...
	public String pushUserRequestFriendMessage(UserRequestFriend userRequestFriend)
			throws PushClientException, PushServerException {
		UserRequestFriendMessage userRequestFriendMessag = new UserRequestFriendMessage(
				userRequestFriend.getUser().getId(), userRequestFriend.getFriend().getId(), userRequestFriend.getMsg());
		PushMessage<UserRequestFriendMessage> pushMessage = new PushMessage<UserRequestFriendMessage>(100,
				userRequestFriendMessag);
